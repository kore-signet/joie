...
#[derive(Default)]
pub struct DatabaseBuilder<Document, DocumentMetadata, SentenceMetadata>
where
    Document: rkyv::Archive + rkyv::Serialize<AllocSerializer<1024>>,
    DocumentMetadata: Pod,
    SentenceMetadata: Archive + rkyv::Serialize<AllocSerializer<1024>> + Default,
{
    sentence_map: HashMap<SentenceId, Sentence<SentenceMetadata>>,
    term_to_sentence: HashMap<u32, Vec<SentenceId>>,
    doc_metadata: BTreeMap<u32, DocumentMetadata>,
    doc_storage: HashMap<u32, Document>,
    make_sentence_metadata: Option<Box<dyn Fn(&str) -> SentenceMetadata>>,
    term_map: TermMap,
}
...
where
    D: rkyv::Archive + rkyv::Serialize<AllocSerializer<1024>> + Clone,
    DM: Pod + Default,
    SM: Archive + rkyv::Serialize<AllocSerializer<1024>> + Default + Clone,
{
    ...
    pub fn build_in(mut self, dir: impl AsRef<Path>) -> io::Result<Database<D, DM, SM>> {
        ...
        let mut metadata_array =
            vec![DM::default(); *self.doc_metadata.last_key_value().unwrap().0 as usize + 1];
